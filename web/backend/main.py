...
def list_proxies():
    ...
    for line in out.splitlines():
        ...
        try:
            ...
            for e in envs:
                if e.startswith("EXIT_COUNTRY="):
                    country = e.split("=", 1)[1]
                if e.startswith("STRICT_NODES="):
                    strict = e.split("=", 1)[1] == "1"
        except Exception:
            pass
        proxies.append({
            "id":         c["ID"],
...
def create_proxy(body: ProxyCreate):
    ...
    env_vars     = (
        f"-e EXIT_COUNTRY={body.country} "
        f"-e STRICT_NODES={'1' if body.strict else '0'}"
    )
    ...
    cmd = (
        ...
        f"{env_vars} "
        f"dperson/torproxy {country_flag} {strict_flag} "
...
def renew_circuit(name: str):
    ...
    idx = get_index(name)
    ctrl_port = BASE_CTRL_PORT + idx
    ok = tor_control(ctrl_port, "NEWNYM")
    if not ok:
